...
NEW_ISO="../isos/borglin.iso"
TMPDIR=mytemper
BORG_TF_CLI_USER="borgtemplate"
# 'terraform_ip' must be in this format: ipv4_address/netmask.
TERRAFORM_IP="192.168.0.220/24"
...
echo "This is a BORG assimilation :-) script to create a Debian"\
    "Installer to make a Template from scratch."
...
cp borg.firewall assimilator/etc/
cp assimilator.sh assimilator/usr/lib/assimilator/
touch assimilator/etc/assimilated.conf

borg_late_script "$TMPDIR" "$ORIGINAL_ISO" "../borg_templates/late_script_borgtemplate.sh" 	\
    "$BORG_TF_CLI_USER" "assimilator"
